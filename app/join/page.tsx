...
export default function JoinPage() {
  ...
  // When the page loads without a user, store the token so we can
  // pick it up automatically after sign-in.
  useEffect(() => {
    if (!token) return;
    if (!user && !loading) {
  ...
  // Once the user is authenticated, attempt to claim — either the token
  // from the URL or one we stored before they signed in.
  useEffect(() => {
  ...
  async function claimInvite(t: string) {
    ...
    if (data.error) {
      setState({ phase: "error", code: data.error as string });
      return;
    }
    ...
    setState({
      ...
      already_linked: data.already_linked ?? false,
    });
  }
      ...
      <div className="cf-page-narrow">
        ...
        <div style={{ display: "flex", gap: "10px", flexWrap: "wrap" }}>
          <button
            ...
            style={{ padding: "12px 28px", background: "var(--accent)", color: "var(--bg)",
              fontWeight: 600, fontSize: "14px", borderRadius: "4px", border: "none", cursor: "pointer" }}
          >
            Sign in
          </button>
          <button
            onClick={() => openAuth("signup")}
            style={{ padding: "12px 28px", background: "var(--surface)", color: "var(--text)",
              fontWeight: 500, fontSize: "14px", borderRadius: "4px",
